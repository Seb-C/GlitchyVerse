Screens.Propulsion = {
	...
	drawScreen: function(controlScreen, x, y, width, height) {
		...
		var minX = ship.objectPositions[propellers[0].id][0], maxX = minX;
		var minY = ship.objectPositions[propellers[0].id][1], maxY = minY;
		...
		for(var i = 1 ; i < propellers.length ; i++) {
			...
			var propellerPosition = ship.objectPositions[propeller.id];
		...
		propellers.map(function(propeller, i) {
			var propellerPosition = ship.objectPositions[propeller.id];
			var propellerSize     = ship.objectSizes    [propeller.id];
